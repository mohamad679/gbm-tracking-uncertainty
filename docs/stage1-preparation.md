...
Date: 2026-09-18. Status: **ARCHIVED EXPLORATORY PILOT; human annotation unavailable.**

On 2026-09-19 the project pivoted to an evaluation design that does not require new human review. Nothing in this record is promoted to reference truth. The viewer and machine proposals are retained as reproducible GlioTrace feasibility artifacts and later qualitative case-study inputs.
...
The former gate required two independent reviewers and adjudication. Those reviewers became unavailable, so this gate was closed as **NOT EXECUTED**, not passed. Quantitative validation moves to datasets with existing expert reference annotations. No inference about GlioTrace tracking accuracy, treatment effects or phenotype switching is authorized from this pilot.
