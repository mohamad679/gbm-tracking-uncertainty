...
## Stable runtime artifacts kept at `docs/` top level

A small set of frozen machine-readable artifacts intentionally keeps its original path because those paths are part of CLI defaults, tests, workflows, hashes, or historical provenance contracts. Moving them would create compatibility churn without improving scientific clarity.

- `dryad-confirmatory-schema-lock.json`
- `external-biological-context-source-manifest.json`
- `stage-c-development-summaries.json`
- `stage-e-dataset-manifest.json`
- `stage-e-development-fit.json`
- `stage-e-protocol.json`
- `stage-e-sequence02-evaluation-lock.json`
- `stage-e-sequence02-evaluation.json`
- `stage-e-split-lock.json`
...
- [`evidence/stage-c/`](evidence/stage-c/) — original Stage C plus the revised T98G-locked Stage C v2 evidence; the runtime-coupled Stage C development summary remains at its stable top-level path.
...
- [`evidence/stage-e/`](evidence/stage-e/) — reader-facing Stage E reports and intermediate evidence that do not require stable runtime paths; core locked JSON artifacts stay at `docs/` top level.
- [`evidence/dryad/`](evidence/dryad/) — local-resumption protocol, runbook, and frozen one-time Dryad machine result; the committed schema lock stays at its stable top-level path.
- [`evidence/external-context/`](evidence/external-context/) — source audit, fallback protocol/result, and combined external-context chronology; the source manifest stays at its stable top-level path.
...
| --- | ---: | --- |
| `KEEP` | 17 | Eight current reader-facing documents plus nine runtime-coupled frozen artifacts keep stable top-level paths. |
| `MOVE_TO_EVIDENCE` | 68 | Protocols, locks, results, stage reports, and engineering evidence moved under `docs/evidence/`. |
...
The cleanup adds this index and the machine-readable audit manifest, so the final top level contains the retained project documents/artifacts plus these navigation files rather than the original flat 100-file layout.

## Claim boundary

The reorganization changes documentation layout only. It does not alter any frozen protocol, locked outcome, historical decision, numerical result, or scientific claim. In particular, Stage E remains `REVISE`, the Dryad `n=3` result remains external technical-transfer evidence, and the repository does not claim human-GBM-wide or clinical validation.
